...
export default function App() {
  ...
  const {
    ...
    showHelp, showMd, showMeta, showSearch, searchQuery, shiftHeld, cmdBuf, message, copied,
    showFilter, filter, matchIds,
    visible, guideData, selIdx, selNode, allTags, markdownText, stats,
    handleKeyDown, setSelectedId, setEditValue, setTagsEditValue, setShowHelp, setShowMd, setShowSearch, setSearchQuery,
    setShowFilter, setFilter,
    startEdit, confirmEdit, cancelEdit,
    openTagEdit, confirmTagEdit, cancelTagEdit,
    toggleCollapse, copyMarkdown,
  } = state;
  ...
  return (
    <div
      ...
      style={{ fontFamily: FONT, fontSize: '14px', backgroundColor: C.bg, color: C.fg,
        height: '100vh', display: 'flex', flexDirection: 'column', outline: 'none',
    ...
    >
        ...
        >
          {visible.map((node, idx) => (
            <OrgNodeRow
              ...
              editRef={editRef}
              editingId={editingId}
              onSelect={() => { setSelectedId(node.id); containerRef.current?.focus(); }}
              onDoubleClick={() => { setSelectedId(node.id); startEdit(node.id); }}
              onEditChange={setEditValue}
              onConfirmEdit={confirmEdit}
              onCancelEdit={cancelEdit}
              onToggleCollapse={() => toggleCollapse(node.id)}
              onOpenTagEdit={() => { setSelectedId(node.id); openTagEdit(node.id); }}
              showMeta={showMeta}
              shiftHeld={shiftHeld}
      ...
      {showSearch && (
        <SearchOverlay
          ...
          onClose={() => { setShowSearch(false); containerRef.current?.focus(); }}
        />
      )}
    </div>
  );
