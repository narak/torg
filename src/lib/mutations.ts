...
export function moveSubtreeDown(nodes: OrgNode[], nodeId: string): OrgNode[] {
  ...
  return [...nodes.slice(0, idx), ...nodes.slice(end, nextEnd), ...nodes.slice(idx, end), ...nodes.slice(nextEnd)];
}
...
export function moveSubtreeUp(nodes: OrgNode[], nodeId: string): OrgNode[] {
  ...
  let end = idx + 1;
  while (end < nodes.length && nodes[end].level > lvl) end++;
  return [...nodes.slice(0, prevIdx), ...nodes.slice(idx, end), ...nodes.slice(prevIdx, idx), ...nodes.slice(end)];
}
