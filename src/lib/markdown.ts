...
import { STATE_ICONS } from '../theme';

export function generateMarkdown(nodes: OrgNode[]): string {
  const lines: string[] = [];
  const parentIds = new Set(
    nodes.slice(0, -1).filter((n, i) => nodes[i + 1].level > n.level).map(n => n.id)
  );

  for (let i = 0; i < nodes.length; i++) {
    const node     = nodes[i];
    const isParent = parentIds.has(node.id);
    const icon     = node.state ? `${STATE_ICONS[node.state]} ` : '';
    ...
    if (isParent) {
      ...
      lines.push(`${hashes} ${icon}${priority}${title}${tags}`);
    } else {
      const indent = '  '.repeat(Math.max(0, node.level - 1));
      lines.push(`${indent}- ${icon}${priority}${title}${tags}`);
    }
  }
  return lines.join('\n').trim();
}
