...
// Hides DONE nodes and their entire subtrees.
function applyHideDone(nodes: OrgNode[]): OrgNode[] {
...
export function applyFilters(
    ...
    const workingNodes = filter.hideDone ? applyHideDone(nodes) : nodes;
    ...
    const empty =
        ...
        !filter.tags.length &&
        !filter.datePreset;
    if (empty) return { filtered: workingNodes, matchIds: new Set() };
    ...
    for (const n of workingNodes) {
        ...
        const priorityOk = !filter.priorities.length || filter.priorities.includes(n.priority);
        const severityOk = !filter.severities.length || filter.severities.includes(n.severity);
        ...
        if (stateOk && priorityOk && severityOk && tagOk && dateOk) matchIds.add(n.id);
    }

    // Include ancestors of matches so the tree context is visible
    const included = new Set<string>();
    for (const id of matchIds) {
        const idx = workingNodes.findIndex((n) => n.id === id);
        if (idx === -1) continue;
        included.add(id);
        const lvl = workingNodes[idx].level;
        for (let i = idx - 1; i >= 0; i--) {
            if (workingNodes[i].level < lvl) {
                included.add(workingNodes[i].id);
            }
            if (workingNodes[i].level === 1) break;
        }
    }

    const filtered = workingNodes.filter((n) => included.has(n.id));
