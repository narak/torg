...
function highlight(text: string, indices: readonly RangeTuple[] | undefined) {
  ...
  for (const [start, end] of mergeRanges(indices)) {
    ...
    parts.push(
      <span key={start} style={{ textDecoration: 'underline', textDecorationColor: C.yellow, color: C.fg }}>
...
function getIndices(result: FuseResult<OrgNode>, key: string, refIndex?: number) {
  const match = result.matches?.find(m => m.key === key && (refIndex === undefined || m.refIndex === refIndex));
  return match?.indices;
}

export function SearchOverlay({ nodes, query, onQueryChange, onSelect, onClose }: SearchOverlayProps) {
  ...
  const results: FuseResult<OrgNode>[] = useMemo(() => {
    ...
    for (const r of fuse.search(query)) {
      ...
      if (!existing) {
        ...
        if (r.score !== undefined && (existing.score === undefined || r.score < existing.score)) {
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)',
        ...
        zIndex: 300, fontFamily: FONT, paddingTop: '12vh' }}
      onClick={onClose}
    >
      <div
        style={{ backgroundColor: C.bgAlt, border: `1px solid ${C.border}`, borderRadius: '4px',
          width: '560px', maxWidth: '92vw', overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 14px',
          ...
          <input
            ...
            style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none',
              color: C.fg, fontFamily: FONT, fontSize: '13px', padding: 0 }}
          />
          {results.length > 0 && (
            <span style={{ color: C.dim, fontSize: '11px', flexShrink: 0 }}>{results.length}</span>
          )}
        </div>
        <div ref={listRef} style={{ maxHeight: '360px', overflowY: 'auto' }}>
          {results.length === 0 ? (
            <div style={{ padding: '12px 14px', color: C.dim, fontSize: '13px' }}>No matches</div>
            ...
            return (
              <div
                ...
                style={{ padding: '6px 14px', display: 'flex', alignItems: 'center', gap: '8px',
                  cursor: 'pointer', backgroundColor: i === cursor ? C.modelineBg : 'transparent',
                  borderLeft: i === cursor ? `2px solid ${C.blue}` : '2px solid transparent' }}
              >
                {'  '.repeat(node.level - 1)}
                {node.state && (
                  <span style={{ fontSize: '11px', flexShrink: 0, color:
                    node.state === 'DONE' ? C.green :
                    node.state === 'DOING' ? C.yellow :
                    node.state === 'WAITING' ? C.violet : C.red }}>
                    {highlight(node.state, getIndices(result, 'state'))}
                  </span>
                )}
                <span style={{ color: C.fg, fontSize: '13px', flex: 1,
        ...
        <div style={{ padding: '4px 14px', borderTop: `1px solid ${C.border}`,
          fontSize: '11px', color: C.dim, display: 'flex', gap: '12px' }}>
          <span>↑↓ navigate</span>
