...
export function Modeline({
  message, cmdBuf,
  tagsEditId, tagsEditValue, allTags, tagsRef,
  onTagsChange, onConfirmTagEdit, onCancelTagEdit,
}: ModelineProps) {
  return (
    <div style={{ backgroundColor: C.modelineBg, borderTop: `1px solid ${C.border}`,
      flexShrink: 0, fontSize: '13px' }}>
      {/* Minibuffer + hints row */}
      <div style={{ backgroundColor: C.bgAlt, padding: '2px 14px', fontSize: '12px',
        minHeight: '20px', borderTop: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: '10px' }}>
        {tagsEditId ? (
          <>
            ...
            <input
              ...
              style={{ background: 'transparent', color: C.green, border: 'none', outline: 'none',
                borderBottom: `1px solid ${C.green}`, fontFamily: FONT, fontSize: '12px', flex: 1, padding: 0 }}
            />
            {allTags.length > 0 && (
              <span style={{ color: C.dim, flexShrink: 0, fontSize: '11px' }}>
                existing: {allTags.slice(0, 8).map((t) => (
                  <span
                    key={t}
            ...
            <span style={{ color: C.dim, flexShrink: 0, fontSize: '11px' }}>Enter=confirm  Esc=cancel</span>
          </>
        ) : (
          <>
            <span style={{ color: cmdBuf ? C.yellow : C.dim, fontSize: '12px', minWidth: '120px',
              overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {message || '\u00a0'}
            </span>
            <span style={{ color: C.border, flexShrink: 0 }}>│</span>
            <span style={{ display: 'flex', gap: '10px', flexWrap: 'nowrap', overflow: 'hidden', fontSize: '11px' }}>
              {HINTS.map(({ key, label }) => (
