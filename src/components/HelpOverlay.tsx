    ...
    >
      <div
        style={{ backgroundColor: C.bgAlt, border: `1px solid ${C.border}`, borderRadius: '4px',
          padding: '22px 30px', maxWidth: '640px', width: '92vw', maxHeight: '80vh', overflowY: 'auto',
          fontSize: '13px', color: C.fg }}
        onClick={e => e.stopPropagation()}
      >
        <div style={{ color: C.blue, fontSize: '14px', marginBottom: '16px' }}>✦  Org-Mode Keybindings</div>
        <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', rowGap: '3px', columnGap: '20px' }}>
          {HELP.map((entry, i) => {
            if (entry.section) return (
              <div key={i} style={{ gridColumn: '1/-1', color: C.magenta, marginTop: i > 0 ? '10px' : '0',
                paddingBottom: '2px', borderBottom: `1px solid ${C.border}` }}>
                {entry.section}
              </div>
            );
            return (
        ...
        <div style={{ marginTop: '16px', color: C.dim, borderTop: `1px solid ${C.border}`,
          paddingTop: '8px', fontSize: '11px' }}>
          Click outside or press <span style={{ color: C.yellow }}>?</span> / <span style={{ color: C.yellow }}>Esc</span> to close
        </div>
      </div>
    </div>
  );
